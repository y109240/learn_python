...
def report_daytime(user_command: str) -> None:
  ...
  if "시간" in user_command:
    return text_to_speech(f"현재 {city}의 시간은 {today.hour}시 {today.minute}분 입니다.")
  elif "날짜" in user_command:
    return text_to_speech(f"오늘 {city}은/는 {today.year}년 {today.month}월 {today.day}일 입니다.")
...
def report_weather(user_command: str) -> None:
  ...
  if "날씨" in user_command:
    city = find_keyword(cities_dict.keys(), user_command, "서울")
    requests_url = f"{BASE_URL}?appid={API_KEY}&q={cities_dict[city]}&lang=Kr"
    response = requests.get(requests_url)
    if response.status_code == 200:
      data = response.json()
      weather = data["weather"][0]["description"]
      temperaturs = round(data["main"]["temp"]-273.15, 2)
      text_to_speech(f"현재 {city}의 날씨는 {weather}, 온도는 {temperaturs}도 입니다.")
    else:
...
def find_keyword(keywords: list[str], sentence: str, default: str = "") -> str:
  ...
  for k in keywords:
    ...
    return default
# "로스앤젤레스와 서울의 시간"이라고 하면 서울이 먼저 출력되는 문제
...
def listen_and_report(command_callbacks: dict[str, Callable[[str], None]]) -> bool:
  ...
  if "종료" in user_command:
    text_to_speech("음성 인터페이스를 종료합니다.")
    return False
  # elif "시간" in user_command:
  #   report_daytime(user_command)
  #   return True
  # elif "날짜" in user_command:
  #   report_daytime(user_command)
  #   return True
  # elif "날짜" in user_command:
  #   report_weather(user_command)
  #   return True
  for k in command_callbacks.keys():
    if k in user_command:
      command_callbacks[k](user_command)
      return True
  else:
...
# 제대로 실행되지 않음
# 덧붙일 내용 : 시간과 날짜, 도시를 두개이상 말할때 순서 정렬
